...
## 5.1 Encoding of unordered categorical data
...
## 5.2 Encoding predictors mith many categories
...
![Plot of the hash collisions simulation](./images/collisionRates.png)
...
locations_sampled <- okc_train %>% dplyr::select(where_town) %>%
distinct(where_town) %>% arrange(where_town)
...
# Takes hash mapping, converts to dataframe, set new columns names,
# calculate hash over name to have original integer value,
# filter for selected cities 
binary_calcs = hash_mapping %>% enframe() %>%
set_names(c('town', 'column_num_16')) %>%
mutate(integer_16 = hashed.value(names(hash_mapping))) %>%
dplyr::filter(town %in% towns_to_sample) %>% arrange(town)
...
# Function used to calculate hash mapping collision rate,
# it can be specified an hash size and if the hash should be signed or not
...
for (iteration_number in 1:10) {
  ...
  result_df = rbind(result_df, c(unsigned_2048, signed_2048, unsigned_4096,
  signed_4096, unsigned_8192, signed_8192, unsigned_16384, signed_16384,
  unsigned_32768, signed_32768, iteration_number))
}
...
rownames(average_df_filtered) <- c("unsigned_2048","signed_2048", "unsigned_4096",
"signed_4096","unsigned_8192", "signed_8192", "unsigned_16384", "signed_16384",
"unsigned_32768", "signed_32768")
...
#Plot of the results
ggplot(data = final_df,aes(x=hashSize, y=CollisionRate))+
geom_line(aes( colour=signed, group=signed)) +
geom_point(aes( colour=signed, group=signed))

```


## 5.3 The problem of novel categories
...
```{r eval=FALSE, library, results="hide", message=FALSE, warning=FALSE}
library(tidyverse)
library(dplyr)
library(embed)
library(ggplot2)
...
```{r odds, include=TRUE, warning=FALSE, eval=FALSE}
load("./Datasets/okc.RData")
...
The shrinking effect is shown below:
...
ggpubr::ggarrange(plot_a, plot_b,
                  labels=c("a.","b."))
```
![Plot a. compares the raw with the shrunken log-odds; plot b. shows the magnitude of the estimates (mean between raw and shrinken odds) versus the difference between the two.](./images/logoddsComparison.png)
...
![Plot of the relationship between the resulting features and the raw log-odds](./images/featuresOdds.png)
...
```{r eval=TRUE}
# Linear and quadratic polynomial contrasts are generated,
# the scores -1,0,1 can be seen as "bad", "medium" and "good" categories
...
# joining all pre-computed features data sets: joins basic dataset,
# binary dataset and basic precomputed features.
